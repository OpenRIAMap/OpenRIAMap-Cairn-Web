...
export function buildReviewPackageFiles(profile: ReviewPackageProfile, draft: ReviewPackageDraft): { files: ReviewPackageFile[]; manifest: ReviewPackageManifest; reviewMarker: ReviewPackageReviewMarker } {
  ...
  for (const feature of draft.features) {
    ...
    pictureBindings.set(`${feature.worldId}\u0000${feature.classCode}\u0000${feature.featureId}`, { worldId: feature.worldId, classCode: feature.classCode, featureId: feature.featureId, kindPath, files: [] });
  }
  ...
  files.push({ path: REVIEW_PACKAGE_LAYOUT.pictureIndexPath, content: json({ schemaVersion: REVIEW_PACKAGE_PICTURE_BINDINGS_SCHEMA_VERSION, bindings: [...pictureBindings.values()].sort((left, right) => `${left.worldId}\u0000${left.classCode}\u0000${left.featureId}`.localeCompare(`${right.worldId}\u0000${right.classCode}\u0000${right.featureId}`)) }) });
