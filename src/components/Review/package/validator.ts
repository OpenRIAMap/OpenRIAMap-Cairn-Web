...
export function validateParsedReviewPackage(parsed: ParsedReviewPackage, profile: ReviewPackageProfile, mode: ReviewPackageValidationMode): ReviewPackageValidationReport {
  ...
  if (parsed.pictureBindingPathPresent) {
    ...
    if (![REVIEW_PACKAGE_PICTURE_BINDINGS_SCHEMA_VERSION, REVIEW_PACKAGE_PICTURE_BINDINGS_LEGACY_SCHEMA_VERSION].includes(bindingSchema) || !Array.isArray(bindingItems)) {
      ...
      if (bindingKeys.size !== featureKeys.size || boundPaths.size !== actualPaths.size) issues.push(issue('PACKAGE_PICTURE_BINDING_INVALID', compat ? 'warning' : 'error', 'Picture binding index does not cover the package features and pictures exactly.', REVIEW_PACKAGE_LAYOUT.pictureIndexPath));
    }
  }
