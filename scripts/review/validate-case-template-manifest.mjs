...
for (const template of manifest.templates ?? []) {
  ...
  const rows = template.paths.flatMap(list).sort().map((file) => `${path.relative(root, file).replaceAll('\\', '/')}:${crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex')}`);
